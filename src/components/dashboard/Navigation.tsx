...
import { GearIcon, RowsIcon } from "@radix-ui/react-icons"; 
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="icon">
          ...
          <GearIcon className="hidden md:block h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0"/>
      ...
      <DropdownMenuContent className="border border-secondary" align="end">
        {links && links.map((link, i) => (
          ...
          {/* <DropdownMenuItem className="hover: cursor-pointer" onClick={() => session?.user?.name ? signOut() : signIn()}>
